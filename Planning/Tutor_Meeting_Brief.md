...
>
> **Day-4 update (2026-04-30, Diagnostic 5 — image-hash leakage).** Sandhya's "no data leakage" sanity check, run as a direct test: compare MD5 of standardised pixel arrays between train and test. 🚨 **The dataset has hard file-level data leakage.** At full scale: **260 within-train MD5 duplicates**, **96 / 1,867 test images (5.1 %) are bit-identical to a training image**, **115 / 1,867 val images (6.2 %) are bit-identical to a training image**, and 1,855 / 1,867 test images (99.4 %) have a training-set feature-NN at cosine similarity > 0.9999. Same magnitude pattern at medium scale. **This finding supersedes the four-step invalidation chain** — every reported metric on this benchmark, ours and prior published, is inflated by an unknown but substantial amount due to file-level duplication and adjacent-slice patient-level overlap that the Islam et al. 2022 dataset release did not document. The paper's headline shifts to: *"we report 99 % on this benchmark and discover the benchmark has 5.1 % bit-identical train/test duplication, calling every reported number on it (ours and prior literature) into question."* Recommended killer experiment: deduplicated re-run (~2-3 hours). See [[Validation_and_Verification]] §3.5.
